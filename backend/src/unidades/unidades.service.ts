...
export class UnidadesService {
    ...
    /**
     * Actualización de estado únicamente (para adminmv)
     */
    async updateStatus(id: string, estado_id: string) {
        // Verificar que la unidad existe
        const existing = await this.prisma.unidades.findUnique({
            where: { id },
        });
        ...
        // Solo actualizar estado en detallesventa
        ...
        // Retornar unidad actualizada
        return this.findOne(id);
    }

    /**
     * Buscar unidad por sectorid
     */
    async findBySectorId(sectorid: string) {
        ...
        if (!unidad) {
            throw new NotFoundException(`Unidad con sectorid '${sectorid}' no encontrada`);
        }
    ...
    /**
     * Actualización completa de unidad (unidades + unidadesmetricas + detallesventa)
    ...
    async updateComplete(id: string, updateDto: UpdateUnidadCompleteDto) {
        ...
        return this.prisma.$transaction(async (tx) => {
            // 1. Actualizar tabla unidades (solo campos no-null del DTO)
            ...
            if (Object.keys(unidadData).length > 0) {
                unidad = await tx.unidades.update({
                    where: { id },
                    data: unidadData,
                });
            } else {
            ...
            if (Object.keys(metricsData).length > 0) {
                await tx.unidadesmetricas.upsert({
                    where: { unidad_id: id },
                    create: {
                        unidad_id: id,
                        ...metricsData,
            ...
            // 3. Upsert detallesventa
            const ventaData: any = {};
            if (updateDto.estado_id !== undefined) ventaData.estado_id = updateDto.estado_id;
            if (updateDto.comercial_id !== undefined) ventaData.comercial_id = updateDto.comercial_id;
            if (updateDto.motivonodisp_id !== undefined) ventaData.motivonodisp_id = updateDto.motivonodisp_id;
            if (updateDto.preciousd !== undefined) ventaData.preciousd = updateDto.preciousd;
            if (updateDto.usdm2 !== undefined) ventaData.usdm2 = updateDto.usdm2;
            if (updateDto.clienteinteresado !== undefined) ventaData.clienteinteresado = updateDto.clienteinteresado;
            if (updateDto.clientetitularboleto !== undefined) ventaData.clientetitularboleto = updateDto.clientetitularboleto;
            if (updateDto.fechareserva !== undefined) ventaData.fechareserva = updateDto.fechareserva ? new Date(updateDto.fechareserva) : null;
            if (updateDto.fechafirmaboleto !== undefined) ventaData.fechafirmaboleto = updateDto.fechafirmaboleto ? new Date(updateDto.fechafirmaboleto) : null;
            if (updateDto.fechaposesion !== undefined) ventaData.fechaposesion = updateDto.fechaposesion ? new Date(updateDto.fechaposesion) : null;
            if (updateDto.obs !== undefined) ventaData.obs = updateDto.obs;
            if (updateDto.tipocochera_id !== undefined) ventaData.tipocochera_id = updateDto.tipocochera_id;
            if (updateDto.unidadcomprador_id !== undefined) ventaData.unidadcomprador_id = updateDto.unidadcomprador_id;

            if (Object.keys(ventaData).length > 0) {
                await tx.detallesventa.upsert({
                    where: { unidad_id: id },
                    create: {
                        unidad_id: id,
                        ...ventaData,
            ...
            // Retornar la unidad actualizada con todas las relaciones
