 ...
 * }
 */
// Overload 1: raw=true -> Retorna ApiResponse<T> completo
export async function apiFetchWrapped<T>(
  endpoint: string,
  options: WrapperRequestOptions & { raw: true }
): Promise<ApiResponse<T>>;

// Overload 2: noThrow=true -> Retorna T | ApiErrorResponse
export async function apiFetchWrapped<T>(
  endpoint: string,
  options: WrapperRequestOptions & { noThrow: true }
): Promise<T | ApiErrorResponse>;

// Overload 3: Default (no raw, no noThrow) -> Retorna T
export async function apiFetchWrapped<T>(
  endpoint: string,
