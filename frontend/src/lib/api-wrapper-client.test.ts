...
describe('api-wrapper-client', () => {
  ...
  describe('apiFetchWrapped', () => {
    it('should successfully fetch and unwrap response', async () => {
      const data = await apiFetchWrapped('/minutas/definitivas/definitivas');
    ...
    it('should return raw response when raw=true', async () => {
      const response = await apiFetchWrapped('/minutas/definitivas/definitivas', { raw: true });
    ...
    it('should include Authorization header with session token', async () => {
      ...
      server.use(
        http.get('http://localhost:3000/minutas/definitivas/definitivas', ({ request }) => {
      ...
      await apiFetchWrapped('/minutas/definitivas/definitivas');
    ...
    it('should include CSRF token for POST requests', async () => {
      ...
      server.use(
        http.post('http://localhost:3000/minutas/definitivas', ({ request }) => {
          capturedHeaders = request.headers;
          return HttpResponse.json({
            success: true,
            data: { id: 'new-id' },
      ...
      await apiFetchWrapped('/minutas/definitivas', { method: 'POST', body: JSON.stringify({}) });
  ...
  describe('apiGet', () => {
    it('should perform GET request and unwrap', async () => {
      const data = await apiGet('/minutas/definitivas/definitivas');
    ...
    it('should accept options', async () => {
      const data = await apiGet('/minutas/definitivas/definitivas', { raw: true });
  ...
  describe('apiPost', () => {
    it('should perform POST request with body', async () => {
      ...
      const data = await apiPost('/minutas/definitivas', newMinuta);
    ...
    it('should stringify body automatically', async () => {
      ...
      server.use(
        http.post('http://localhost:3000/minutas/definitivas', async ({ request }) => {
      ...
      await apiPost('/minutas/definitivas', { test: 'value' });
  ...
  describe('apiPatch', () => {
    it('should perform PATCH request', async () => {
      server.use(
        http.patch('http://localhost:3000/minutas/definitivas/minuta-1', async ({ request }) => {
      ...
      const data = await apiPatch('/minutas/definitivas/minuta-1', { estado: 'APROBADA' });
  ...
  describe('apiPut', () => {
    it('should perform PUT request', async () => {
      server.use(
        http.put('http://localhost:3000/minutas/definitivas/definitivas/project-1', async ({ request }) => {
          const body = await request.json();
          return HttpResponse.json({
            success: true,
      ...
      const data = await apiPut('/minutas/definitivas/definitivas/project-1', { nombre: 'Updated Name' });
  ...
  describe('apiDelete', () => {
    it('should perform DELETE request', async () => {
      server.use(
        http.delete('http://localhost:3000/minutas/definitivas/minuta-1', () => {
          return HttpResponse.json({
            success: true,
      ...
      const data = await apiDelete('/minutas/definitivas/minuta-1');
